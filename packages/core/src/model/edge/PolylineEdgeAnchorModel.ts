...
import {
  ...
  segmentDirection,
  points2PointsList,
  pointFilter, getClosestPointOfPolyline,
} from '../../util/edge';
import RectNodeModel from '../node/RectNodeModel';
import BaseEdgeAnchorModel from './BaseEdgeAnchorModel';

type BaseEdgeModelId=string;
export { PolylineEdgeAnchorModel };
export default class PolylineEdgeAnchorModel extends BaseEdgeAnchorModel {
  ...
  removeCrossPoints(startIndex, endIndex, pointList) {
    ...
    if (endIndex === pointList.length - 2 && this.targetNode instanceof BaseNodeModel) {
      const start = list[startIndex];
      const end = list[endIndex];
  ...
  updateCrossPoints(pointList) {
    ...
    if (targetNode instanceof BaseNodeModel) {
  ...
  updatePoints() {
    const pointsList = getPolylinePoints(
      ...
      this.sourceNode,
      this.targetNode as BaseEdgeAnchorModel,
      this.offset || 0,
    );
  ...
  dragAppend(appendInfo, dragInfo) {
    ...
    if (direction === SegmentDirection.HORIZONTAL) {
      ...
      if (endIndex === this.pointsList.length - 1) {
        ...
        let inNode;
        if (this.targetNode instanceof BaseNodeModel) {
          inNode = isInNode(endPosition, this.targetNode);
        }
        if (this.targetNode instanceof BaseEdgeAnchorModel) {
          inNode = isInEdge(endPosition, this.targetNode);
        }
        if (!inNode) {
  ...
  */
  ...
  getDefaultAnchor() {
    const { pointsList, properties } = this;
    const edgePointLengthList = [];
    let pointRemark = null;
    pointsList.forEach((arr, index) => {
      if (!pointRemark) {
        pointRemark = arr;
      } else if (edgePointLengthList.length === 0) {
        edgePointLengthList.push({
          startX: pointRemark.x,
          startY: pointRemark.y,
          endX: arr.x,
          endY: arr.y,
          startLength: 0,
          endLength: Math.abs(arr.y - pointRemark.y + arr.x - pointRemark.x),
        });
        pointRemark = arr;
      } else {
        edgePointLengthList.push({
          ...
          endX: arr.x,
          endY: arr.y,
          startLength: edgePointLengthList[index - 2].endLength,
          endLength: edgePointLengthList[index - 2].endLength
            + Math.abs(arr.y - pointRemark.y + arr.x - pointRemark.x),
        });
        pointRemark = arr;
      }
    });
    const anchorList = [];
    if (edgePointLengthList.length > 0) {
      properties?.staticAttribute?.anchor.forEach((arr, index) => {
        const station = edgePointLengthList[edgePointLengthList.length - 1].endLength
          * arr.percentage;
        for (let i = 0; i < edgePointLengthList.length; i++) {
          if (station >= edgePointLengthList[i].startLength
            ...
            anchorList.push({
              ...
              x: edgePointLengthList[i].startX === edgePointLengthList[i].endX
                ...
                : (edgePointLengthList[i].startX < edgePointLengthList[i].endX
                  ? edgePointLengthList[i].startX + station - edgePointLengthList[i].startLength
                  : edgePointLengthList[i].startX - station + edgePointLengthList[i].startLength),
              // eslint-disable-next-line no-nested-ternary
              y: edgePointLengthList[i].startY === edgePointLengthList[i].endY
                ? edgePointLengthList[i].startY
                : (edgePointLengthList[i].startY < edgePointLengthList[i].endY
                  ? edgePointLengthList[i].startY + station - edgePointLengthList[i].startLength
                  : edgePointLengthList[i].startY - station + edgePointLengthList[i].startLength),
            });
            break;
          }
    ...
    for (let i = 0; i < this.graphModel.edges.length; i++) {
      ...
      if (edgeModel.id === edgeId) {
        ...
        this.graphModel.edges.forEach((edge) => {
          ...
          const findEdge = anchors.find((val) => val.id === edge.targetAnchorId);
          if (findEdge) {
            edge.updateEndPoint(findEdge);
            // 如果是自定义边文本位置，则移动节点的时候重新计算其位置
            if (edge.customTextPosition === true) {
              edge.resetTextPosition();
            } else if (edge.modelType === ModelType.POLYLINE_EDGE && edgeModel.text?.value) {
